...
class DateObjectMixin(YearMixin, MonthMixin, DateMixin):
    ...
    def get_object(self, queryset=None):
        ...
        date = _date_from_string(
            year, self.get_year_format(), month, get_month_format()
            )
        if queryset is None:
            queryset = self.get_queryset()
        if (not self.get_allow_future() and date > date.today()):
            raise Http404("Future {} is not allowed because {}.allow_future is False").format((queryset.model._meta.verbose_name_plural), 
